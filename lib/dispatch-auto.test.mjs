...
const { readRunStates, dependentsOf, runLooksFinished, readCompletedTickets, readLatestStates, backUpStatus, lostHistory, hasLiveForeman } = await jiti.import("./dispatch-auto.ts");
...
test("a verdict written beside the word done still means done", (t) => {
  // A reviewer ticket finishes by recording a FAIL verdict; the ticket itself is
  // complete. Reading the cell as "not done" re-dispatched T127 and T160 eight
  // times, each round reaching the same conclusion.
  const dir = project(t, `## Run now
| Ticket | State | Files changed | Verify |
|---|---|---|---|
| T160 | done（round-5 verdict FAIL；reviewer 判 overall_correctness=correct） | artifacts | exit=1 by design |
| T127 | done (runtime BLOCKED by design) | — | pass |
| T99 | not done | — | — |
`);
  const done = readCompletedTickets(dir);
  assert.equal(done.has("T160"), true);
  assert.equal(done.has("T127"), true);
  assert.equal(done.has("T99"), false, "a cell reading 'not done' is not done");
});

test("a forced ticket runs again even though status.md calls it done", (t) => {
  // T160 is a reviewer ticket: it finished by recording FAIL. Once the things it
  // judged were fixed, it had to judge again, and completion alone would have
  // kept it out of every future batch.
  const dir = project(t, `## Run now
| Ticket | State | Files changed | Verify |
|---|---|---|---|
| T160 | done（verdict FAIL） | artifacts | exit=1 by design |
| T161 | done | spec | pass |
`);
  const done = readCompletedTickets(dir);
  assert.equal(done.has("T160"), true, "it is genuinely recorded as done");
  // What the loop does with that is to subtract the forced ids before selecting.
  const forced = new Set(["T160"]);
  const satisfied = [...done].filter((id) => !forced.has(id));
  assert.equal(satisfied.includes("T160"), false, "a forced ticket is not treated as satisfied");
  assert.equal(satisfied.includes("T161"), true, "everything else still counts");
});

test("a verdict with the foreman's own words after it is still that verdict", (t) => {
  // T14/T15/T18/T26 came back "needs-decision（誠實停在基建停止點）". The reader
  // demanded a closing pipe straight after the word, saw nothing, and the loop
  // re-dispatched the whole batch instead of leaving those tickets behind.
  const dir = project(t, `## Run now
| Ticket | State | Files changed | Verify |
|---|---|---|---|
| T14 | needs-decision（票內審計完成，誠實停在基建停止點） | HANDOFF.md | — |
| T26 | blocked（上游未交付） | — | fail |
| T8 | done（首派 frontend 失敗；worker 重試完成） | src/a.tsx | pass |
| T3 | skipped（depends T2） | — | — |
`);
  const states = readRunStates(dir);
  assert.equal(states.get("T14"), "needs-decision");
  assert.equal(states.get("T26"), "blocked");
  assert.equal(states.get("T8"), "done");
  assert.equal(states.get("T3"), "skipped");
  const latest = readLatestStates(dir);
  assert.equal(latest.get("T14"), "needs-decision");
  assert.equal(latest.get("T8"), "done");
});

test("a live foreman process is proof the run is not over", () => {
  // The signal that failed was file mtime: T16 was silent for eleven minutes
  // during one long turn, the loop called the run dead, and dispatched a second
  // foreman onto the same four tickets.
  const CWD = "/tmp/ompweb-no-such-project-" + Math.random().toString(36).slice(2);
  assert.equal(hasLiveForeman(CWD), false, "no process, no run");
  // This very test process is not an omp foreman, so a cwd that exists is still false.
  assert.equal(hasLiveForeman(process.cwd()), false);
});

test("the silence window is long enough for one slow turn", async () => {
  const mod = await jiti.import("./dispatch-auto.ts");
  // Five minutes was shorter than a single observed model turn.
  const dir = mkdtempSync(join(tmpdir(), "ompweb-silence-"));
  const sessionFile = join(dir, "s.jsonl");
  writeFileSync(sessionFile, "{}");
  const now = Date.now();
  const eleven = now - 11 * 60_000;
  utimesSync(sessionFile, new Date(eleven), new Date(eleven));
  assert.equal(mod.runLooksFinished({ startedAt: now - 30 * 60_000, sessionFile }, now), false,
    "eleven minutes of quiet must not read as finished");
