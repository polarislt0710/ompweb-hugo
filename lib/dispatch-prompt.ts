...
5a. Read @${HANDOFF_DIR}/decisions.md if it exists. It is the owner's answers to questions this plan could not settle, and it carries the authority the plan reserves for a person: a path it authorises may be read, a migration it approves may be run. It never widens a ticket beyond what it names — an entry about one ticket says nothing about any other, and silence there is not permission. A ticket that stopped at a gate the owner has since opened should be run, not skipped.
