...
test("a saved session carries storage, not only cookies", () => {
  const slug = saved("storage.example");
  // The first real site tried kept its token in localStorage and set no cookie
  // of its own, so a session that only knows about cookies is not a session.
  assert.equal(readSavedSession(slug), null, "nothing saved yet should read as no session");
  writeFileSync(join(process.env.OMP_WEB_CAPTURE_PROFILE_DIR, `${slug}.session.json`), JSON.stringify({
    cookies: [],
    origins: [{ origin: "https://storage.example", local: [["token", "abc"]], session: [] }],
  }));
  const session = readSavedSession(slug);
  assert.equal(session.origins[0].local[0][1], "abc");
  deleteCaptureProfile(slug);
});
