...
test("detects sandbox: paths used by Codex / Claude Code file links", async () => {
  const { isFilePathReferencedByEntries } = await loadSubject();
  const filePath = "/var/folders/g7/tmp/omp-image.webp";
  const entries = [
    {
      type: "message",
      id: "entry-1",
      ...
      message: {
        ...
        content: [
          {
            type: "text",
            text: `[download](sandbox:${filePath})`,
          },
        ],
      },
    },
  ];
  assert.equal(isFilePathReferencedByEntries(filePath, entries), true);
});
