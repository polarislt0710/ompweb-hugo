...
test("exposes the expected command set with hints and arg requirements", async () => {
  ...
  assert.deepEqual(names, ["goal", "plan", "flow", "wait-what", "grill", "ask-matt", "review", "fix", "test", "explain", "simplify", "commit", "advisor", "loop", "agy"]);
...
test("agy delegates to the official Antigravity CLI skill", async () => {
  const { getWebSlashCommand, expandWebSlashCommand } = await loadSubject();
  const agy = getWebSlashCommand("agy");
  assert.ok(agy);
  assert.equal(agy.requiresArgs, false);
  assert.match(agy.buildPrompt("reset the password"), /antigravity-cli skill/);
  assert.match(agy.buildPrompt("reset the password"), /reset the password/);
  assert.match(agy.buildPrompt("reset the password"), /do not use the google-antigravity provider/);
  const empty = expandWebSlashCommand("/agy");
  assert.equal(empty.kind, "expand");
  if (empty.kind === "expand") {
    assert.match(empty.prompt, /antigravity-cli skill/);
    assert.match(empty.prompt, /current conversation/);
  }
});
